...
module.exports = (() => {
    ...
    class VKBot {
        ...
        listen() {
            this.callApi('groups.getLongPollServer', this.token, { group_id: this.groupId }, data => {
                const { response } = data
                if (response) {
                                                ...
                                                if (matches) {
                                                    for (let commandObj of self.commands) {
                                                        if (matches[1].toLowerCase() === commandObj.command) {
                                                            let values = matches[2] ? matches[2].toLowerCase() : undefined
                                                            commandObj.action(userId, values)
                                                        }
                                                    }
                                                }
                                            }
        ...
        response(userId, message) {
            this.callApi('messages.send', this.token, {
                user_id: userId,
                message: message
            });
        }

        callApi(method, token, parameters) {
